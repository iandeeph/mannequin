<?php
	...
	if(isset($_POST['btnLogin'])){
		...
		$loginQry = "SELECT * FROM tb_user WHERE username = '".$postUsername."' AND password = '".md5($postPassword).md5(md5($postPassword))."'";
		if($resultLogin = mysqli_query($conn, $loginQry)){
			if (mysqli_num_rows($resultLogin) != 0) {
    			...
    			logging($now, $postUsername, "User Login Success", $logingContentText, $rowLogin['idUser']);
				header('Location: ./');
			}else{
