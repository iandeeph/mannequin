<?php
	...
	if(isset($_POST['btnDeleteVisitor'])){
		foreach ($_POST['checkboxVisitor'] as $selectedIdVisitor) {
			$delVisitorQry = "DELETE FROM tb_message WHERE idMsg = '".$selectedIdVisitor."'";

			// ======================================== LOGGING =============================
				$nameDelVisitorQry = "";
	            $nameDelVisitorQry = "SELECT idMsg, date, firstName, lastName, phone, email, message FROM tb_message WHERE idMsg = '".$selectedIdVisitor."'";
	            if($resultDelVisitorQry = mysqli_query($conn, $nameDelVisitorQry)){
	                if (mysqli_num_rows($resultDelVisitorQry) > 0) {
	                    while($rowDelVisitors = mysqli_fetch_array($resultDelVisitorQry)){
	                        $idDelVisitors    		= $rowDelVisitors['idMsg'];
	                        $datePostDelVisitors  	= $rowDelVisitors['date'];
	...
	<div class="col s12">
		<form action="#" method="post" enctype="multipart/form-data">
			...
			<table class="highlight responsive-table">
				...
				<tbody>
					<?php
						if($resultVisitorQry = mysqli_query($conn, "SELECT *, DATE_FORMAT(date,'%e %b %Y - %H:%i') as datepost FROM tb_message ORDER BY date DESC")){
							if (mysqli_num_rows($resultVisitorQry) > 0) {
								while ($rowVisitor = mysqli_fetch_array($resultVisitorQry)) {
									$idvisitor         	= $rowVisitor['idMsg'];
